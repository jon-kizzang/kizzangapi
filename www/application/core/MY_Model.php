...
class MY_Model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        array_unshift($this->before_create, 'modifiable_attributes');
        array_unshift($this->before_update, 'modifiable_attributes');
        ...
        $this->memcacheEnable = $config->item( 'memcache_enable' );
        $this->memcacheExpired = $config->item( 'memcache_timeout' );
        
        $memcacheServers = $config->item( 'memcache_servers' );
        
         $this->memcacheInstance = new Memcached(); //get_class($this)
         if(!count($this->memcacheInstance->getServerList()))
             $this->memcacheInstance->addServers($memcacheServers);

        $this->_temporary_return_type = $this->return_type;

        // set form validation memcached
        $this->form_validation->memcacheInstance = $this->memcacheInstance;
        $this->form_validation->memcacheEnable = $this->memcacheEnable;
    }
    ...
    public function maintenanceMode() {
      ...
      # in mysql format YEAR-MONTH-DAY
      $startMaintenace = "2015-03-15 00:00:00" ;
      $endMaintenace = "2015-03-16 00:00:00" ;

      $start = strtotime( $startMaintenace ) ;
      $end = strtotime( $endMaintenace ) ;
      $now = time();


      //are we in a maintenace mode
      if ($now > $start && $now < $end) {

        return $endMaintenace;
    ...
    public function insert($data, $skip_validation = FALSE)
    {
        ...
        if ($data !== FALSE)
        {
            ...
            $insert_id = ($success) ? $this->_database->insert_id() : 0;
    ...
    /**
     * remove element if not exists in public_attributes array
     */
    public function modifiable_attributes($row) {

        if ( ! empty( $this->public_attributes ) )
        {
            $public_attributes = array_map('strtolower', $this->public_attributes);
            $row = array_change_key_case($row);

            foreach ($row as $key=>$value)
            {
                if ( !in_array($key, $public_attributes) )
                {
                    if (is_object($row))
                    {
                        unset($row->$key);
    ...
    protected function _set_where($params)
    {
        if (count($params) == 1 && is_array($params[0]))
        {
            foreach ($params[0] as $field => $filter)
            {
                if (is_array($filter))
                {
                    if ( array_key_exists( 'isRaw', $filter ) )
                    {
                        $this->_database->where($field, $filter['isRaw'], FALSE );
                    }
                    else
                    {   
